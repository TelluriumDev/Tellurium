...
target("TSEssential") -- Change this to your mod name.
    ...
    set_configdir("$(buildir)/includes")
    set_configvar("TSET_WORKSPACE_FOLDER", "$(projectdir)")
    add_configfiles("src/(includes/Versions.h.in)")
    add_includedirs("src", "$(buildir)/includes/includes")
    set_exceptions("none")
    set_kind("shared")
    set_languages("c++23")
    set_symbols("debug")
